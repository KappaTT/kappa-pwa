...
class Checkbox extends React.Component {
  ...
  renderLabel() {
    const { label, disabled, flexDirection, image, labelStyle, imageStyle, styles } = this.props;
    ...
    const imageStyles = [styles.imgStyles, imageStyle, flexDirection && this.spaceAround(flexDirection)];

    if (image && !label) return <Image style={imageStyles} source={{ uri: image }} />;
  ...
  renderChecked() {
    ...
    if (this.state.checked) return <Icon name={iconName} family={iconFamily} color={iconColor} size={iconSize} />;
